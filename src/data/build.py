...
def build_ss_train_loader(cfg, mapper):
    ...
    in `cns/config/config.py` or in a custom `configs/*.yaml` config file
...
def build_ss_datasets(cfg):
    ...
    if cfg.DATASETS.MODE == "CROSS_DATASET":
        ...
        labeled_dataset_dicts, unlabeled_dataset_dicts = split_labeled_dataset(
            labeled_dataset_dicts, cfg
        )
    else:
...
def split_labeled_dataset(dataset_dicts, cfg):
    ...
    if cfg.DATASETS.SPLIT_USE_PREDEFINED:  # Load the pre-defined split from file
        ...
        assert (
            len(labeled_ids) > 0
        ), "The list of ids in the cfg.DATASETS.SPLIT_PATH is empty."
        ...
        cnt_labeled = int(
            images_count * cfg.DATASETS.SPLIT_SUP_PERCENT / 100
        )  # Calculate the size of labeled subset
            ...
            def generate_batch(d_iter, buckets, batch_size):
                ...
                while True:  # Repeat until one of the buckets get filled
                    for bucket_id in [
                        0,
                        1,
                    ]:  # Check if some bucket has images enough for the batch
                ...
                # Unreachable code
                raise RuntimeError(
                    "Dataset should be of infinite size due to the sampler"
                )

            labeled_batch = generate_batch(
                labeled_d_iter, self._labeled_buckets, self.labeled_batch_size
            )
            unlabeled_batch = generate_batch(
                unlabeled_d_iter, self._unlabeled_buckets, self.unlabeled_batch_size
            )
...
def build_detection_train_loader(cfg):
    ...
    # CNS: subsample the dataset if needed
...
def check_subsample_dataset(dataset_dicts, cfg):
    ...
    if (
        cfg.DATASETS.MODE == "RANDOM_SPLIT"
    ):  # Check if the dataset should be subsampled according to config
        # Reuse CNS code, ignore the unlabeled split
